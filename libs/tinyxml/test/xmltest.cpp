...
int main()
{
...

    {
        ...
        const char *demoEnd = "<?xml version=\"1.0\" standalone=\"no\" ?>"
                              "<!-- Our to do list data -->"
                              "<ToDo>"
                              "<!-- Do I need a secure PDA? -->"
...
// The example parses from the character string (above):
        ...
        outputStream << doc;
        XmlTest("Output stream correct.", string(demoEnd).c_str(), outputStream.str().c_str(),
                true);
#endif
...

    {
        ...
        const char *expected = "Line 5 has \"quotation marks\" and 'apostrophe marks'. It also has "
...

    {
        ...
        XmlTest("Parsing repeated attributes.", true,
                doc.Error()); // is an  error to tinyxml (didn't use to be, but caused issues)
        // XmlTest( "Parsing repeated attributes.", "blue", doc.FirstChildElement( "element"
        // )->Attribute( "attr" ) );
    }
...
#ifdef TIXML_USE_STL
    {
        ...
        XmlTest("Comments ignore entities.", " declarations for <head> & <body> ", c0->Value(),
                true);
